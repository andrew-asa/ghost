...
// @namespace         https://greasyfork.org/zh-CN/users/707819-xiaobai-andrew
...
var  url = document.location.toString();
   ...
   var title="";
    if($("#title").length >0 && $("#info").length>0 && $("table").length >0){
        console.log("安装下载按钮");
        title = $("#title").text();
        $("#title").append("<a href='#' id='downloadTxt'>下载</a><div id='downloadTxt'></div>");
    }
    $("#downloadTxt").click(function(){
        var st="";
        var stl=[];
        var bookUrl=getBookUrl();
        console.log("正在下载");
        $('table').find('tbody').each(function () {
            $(this).find('tr').each(function () {
            	...
            	$(this).find('td').each(function () {
                    ...
                    st = st + text + "##" + url +"\n";
                    stl.push({"title":text, "url":url
                             })
                    if(!isNull(url)){
                       // var v = $(this).load(url+" #content");
                    }
            	});
            });
console.log(st);
